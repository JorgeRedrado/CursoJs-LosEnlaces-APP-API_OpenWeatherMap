...
const LlamarAPI = url => {
  $.ajax({
    ...
    success: function(response) {
      ...
      <figure class="figure" style="background-color: aquamarine; width:100%">    
      <img src="${rutaicono}" width="200px" class="figure-img img-fluid rounded" alt="${descripcion}">               
...
const PrepararURL = () => {
  ...
  let url = `https://api.openweathermap.org/data/2.5/${tipoConsulta}?`; //forecast //weather
  url += `q=${ciudad.val()},es`;
  url += `&appid=${key}`;
  url += `&units=metric`;
  url += `&lang=es`;
